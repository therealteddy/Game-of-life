...
void GetCellArray(unsigned int*** Matrix, unsigned int cw, unsigned int ch, unsigned int gw, unsigned int gh) {
    *Matrix = malloc((gh/ch) * sizeof(unsigned int*)); 
    for (int i = 0; i < (gh/ch); *(*Matrix+(i++)) = calloc((gw/cw), sizeof(unsigned int)));
} 

void PrintCellArray(unsigned int** Matrix, unsigned int cw, unsigned int ch, unsigned int gw, unsigned int gh) {
    unsigned int j, i; 
    for(i = 0; i++ < 6; printf(" "));
    for(i = 0; i < (gw/cw); printf("[%d]", i++));
    printf("\n");
    for (j = 0; j < (gh/ch); ++j) {
        printf("[%d] [  ", j); 
        for (i = 0; i < (gw/cw); printf("%d  ", Matrix[j][i++]));
        printf("]\n");
    } 
}
...
void DrawCellGrid(int XPos, int YPos, unsigned int GridWidth, unsigned int GridHeight, unsigned int CellWidth, unsigned int CellHeight, Color COLOR) {
    for (int i = XPos; i <= XPos+GridWidth; i+=CellWidth) {
        DrawLine(i, YPos, i, YPos+GridHeight, COLOR);
    }
    for (int i = YPos; i <= YPos+GridHeight; i+=CellHeight) {
        DrawLine(XPos, i, XPos+GridWidth, i, COLOR);
    }
}
